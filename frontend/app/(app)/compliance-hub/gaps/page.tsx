...
export default function ComplianceGapsPage() {
  ...
  const [appliedQueryParam, setAppliedQueryParam] = useState(false);
  ...
  useEffect(() => {
    if (appliedQueryParam) return;
    const fw = searchParams.get("framework_key");
    if (fw) {
      setFrameworkKey(fw);
    }
    setAppliedQueryParam(true);
  }, [appliedQueryParam, searchParams]);
  ...
  const openGap = (gap: GapItem) => {
    setSelectedControlKey(gap.control_key);
  };
    ...
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto min-w-0">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">Prioritize controls below the implementation threshold.</p>
        </div>
        ...
        {gaps.length > 0 && (
          <>
            <div className="flex items-center gap-2">
              ...
              >
                <option value="score_asc">Implementation ↑</option>
                <option value="score_desc">Implementation ↓</option>
              ...
              <table className="w-full min-w-[900px] border-collapse text-sm">
                <thead className="sticky top-0 z-[1] bg-muted/95 backdrop-blur supports-[backdrop-filter]:bg-muted/80">
                  <tr className="border-b">
                    ...
                    <th className="text-left font-medium text-muted-foreground py-3 px-4 w-[110px]">Implementation</th>
