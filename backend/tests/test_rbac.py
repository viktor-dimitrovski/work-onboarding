...
def test_role_protected_route_access(client: TestClient) -> None:
    ...
    forbidden = client.get('/api/v1/users', headers=tenant_headers(employee_tokens['access_token']))
    assert forbidden.status_code == 403

    admin_tokens = login(client, 'seed-admin@example.com')
    allowed = client.get('/api/v1/users', headers=tenant_headers(admin_tokens['access_token']))
